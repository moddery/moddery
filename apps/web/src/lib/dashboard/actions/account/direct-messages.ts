...
import {
  ...
  MARK_DIRECT_THREAD_READ_MUTATION,
...
import {
  ...
  type MarkDirectThreadReadMutationData,
  type MarkDirectThreadReadMutationVariables,
...
export async function markDirectThreadRead(
  ...
  const { data } = await apolloClient.mutate<
    MarkDirectThreadReadMutationData,
    MarkDirectThreadReadMutationVariables
  >({
    mutation: MARK_DIRECT_THREAD_READ_MUTATION,
    variables: { threadId },
  });

  if (!data?.markDirectThreadRead) {
    throw new Error('Direct thread read state did not return from the API');
  }

  return data.markDirectThreadRead;
}
